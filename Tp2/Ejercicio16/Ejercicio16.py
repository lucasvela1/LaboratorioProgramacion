...
class ArticuloInvalidoErrror(Exception):  #Clase solo para exepción
...
class articulo:
    def __init__(self, titulo, autor, texto):
        titulo = titulo.strip()
        texto= texto.strip() #Elimina espacios antes y después del string, por lo que no elimina espacios entre palabras
        if len(titulo) < 10:
            raise ArticuloInvalidoErrror("El título debe tener al menos 10 caracteres.")
        if len(texto) < 10:  
            raise ArticuloInvalidoErrror("El texto debe tener al menos 10 caracteres.")
        self.titulo= titulo
...
class parser_html ():
    ...
    def generar_html(self):
        ...
        for autor,lista in articulos_por_autor.items():
           tabla_autores+=f"<tr><td>{autor}</td><td>{len(lista)}</td></tr>\n"
        tabla_autores+="""
                </tbody>
            </table>
        </div>
